...
class RosAriaNode
{
  ...
  public:
    ...
    void cmdGripper_cb( const std_msgs::StringConstPtr &); //Para enviar comandos a la pinza
    void cmdPTU_cb( const std_msgs::StringConstPtr &); // Para enviar comandos a la PTU
  ...
  protected:
    ...
    ros::Publisher poseGripper_pub; // estado de la pinza
    ros::Publisher posePTU_pub; // estado de la PTU
    ...
    ros::Subscriber cmdGripper_sub; // Suscriptor pinza
    ros::Subscriber cmdPTU_sub; // Suscriptor PTU
    ...
    ros::Time veltime;
    ros::Time gripperTime; // tiempo en que llegan comandos para la pinza
    ros::Time ptuTime;  // tiempo en que llegan comando para la PTU
    ...
    ArRobot *robot;
    ArGripper *gripper; //Puntero a pinza
    bool haveGripper; // Para ver si hay pinza
    PTU* ptu; //Puntero a PTU
    bool havePTU;   // Ver si hay PTU
...
//Clase para ver el estado de la pinza copiada del ejemplo
...
//Clase para ver el estado de la PTU copiada del ejemplo
// Adds robot callback to print PTU status.
class PrintPTUStatus
{
  PTU* myPTU;
  ArFunctorC<PrintPTUStatus> myPrintCB;
public:
    PrintPTUStatus(PTU* ptu) :
    myPTU(ptu),
    myPrintCB(this, &PrintPTUStatus::printStatus)
  {
  }

  void addRobotTask(ArRobot* robot)
  {
    robot->addUserTask("PrintPTUStatus", 10, &myPrintCB);
  }
...
void RosAriaNode::readParameters()
{
  ...
  else
  {
    ...
    //n_.setParam( "TicksMM", TicksMM); //ELIMINAR COMENTARIO
  }
  ...
  else
  {
    ...
    //n_.setParam( "DriftFactor", DriftFactor); //ELIMINAR COMENTARIO
  }
...
int RosAriaNode::Setup()
{
  ...
  // ArGripper gripper(robot); objeto pinza
  ...
  readParameters();
  haveGripper=robot->getOrigRobotConfig()->getHasGripper();

  if(haveGripper)
  {
    //Copiado ejemplo pinza
   ...
   else ArLog::log(ArLog::Normal, "Robot does not indicate that it has a gripper.");

   havePTU = ptu->getHavePTU();
   if(havePTU)
   {
     //Copiado ejemplo pinza
     ptu->status();
     // if(ptu->getType() == ArGripper::NOGRIPPER)
     //     {
     //         ArLog::log(ArLog::Terse, "gripperExample: Error: Robot does not have a gripper. Exiting.");
     //         Aria::exit(-1);
     //         return -1;
    ...
    else ArLog::log(ArLog::Normal, "Robot does not indicate that it has a pix.");
  ...
  // suscribirse a comandos para la pinza
  cmdGripper_sub = n.subscribe( "cmd_gripper", 1, (boost::function <void(const std_msgs::StringConstPtr &msg)>)
      boost::bind(&RosAriaNode::cmdGripper_cb, this, _1 ));

  // suscribirse a comandos para la pinza
  cmdPTU_sub = n.subscribe( "cmd_ptu", 1, (boost::function <void(const std_msgs::StringConstPtr &msg)>)
      boost::bind(&RosAriaNode::cmdPTU_cb, this, _1 ));
...
// Enviar comandos a la pinza
void RosAriaNode::cmdGripper_cb( const std_msgs::StringConstPtr &msg)
//void RosAriaNode::cmdpinza_cb( const geometry_msgs::TwistConstPtr &mensaje)
{
  gripperTime = ros::Time::now();
  ROS_INFO( "nueva orden: [%s](%0.3f)", msg->data.c_str(), gripperTime.toSec() );
  bool esOrdenCorrecta=false;

  if(strcmp("subir", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es subir pinza");
    if(haveGripper)
    {
        ArLog::log(ArLog::Normal, "Moving gripper lift up...");
        gripper->liftUp();
    }
    else ROS_WARN("No se envia orden %s porque no se ha detectado la pinza",msg->data.c_str());
    esOrdenCorrecta=true;
  }

  if(strcmp("bajar", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es bajar pinza");
    if(haveGripper)
    {
  ...
  if(strcmp("parar pinza", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es parar pinza");
     if(haveGripper)
     {
     ...
     // stops both lift an grip
    //myGripper->liftStop(); // stops just the lift
    //myGripper->gripStop(); // stops just the gripper
     esOrdenCorrecta=true;
  }

  if(strcmp("abrir", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es abrir pinza");
    if(haveGripper)
    {
        ArLog::log(ArLog::Normal, "Openning gripper...");
        gripper->gripOpen();
    }
    else ROS_WARN("No se envia orden %s porque no se ha detectado la pinza",msg->data.c_str());
    esOrdenCorrecta=true;
     }
    if(strcmp("cerrar", msg->data.c_str())==0)
     {
        ROS_INFO("La orden es cerrar pinza");
        if(haveGripper)
        {
      ...
      robot->setRotVel(msg->angular.z*180/M_PI);
      robot->unlock();*/
      if(!esOrdenCorrecta) ROS_WARN("La orden %s no es correcta. Utilice abrir, cerrar, subir, bajar o parar", msg->data.c_str());


}

// Enviar comandos a la PTU
void RosAriaNode::cmdPTU_cb( const std_msgs::StringConstPtr &msg)
{
  ptuTime = ros::Time::now();
  ROS_INFO( "nueva orden: [%s](%0.3f)", msg->data.c_str(), ptuTime.toSec() );
  bool esOrdenCorrecta=false;

  if(strcmp("derecha", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es girar ptu a la derecha");
    if(havePTU)
    {
        float currentAngle = ptu->getPan() +15;
        ArLog::log(ArLog::Normal, "Moving PTU to the right 15º...");
        ptu->gotoPos(currentAngle,0);
    }
    else ROS_WARN("No se envia orden %s porque no se ha detectado la PTU",msg->data.c_str());
    esOrdenCorrecta=true;
  }

  if(strcmp("izquierda", msg->data.c_str())==0)
  {
    ROS_INFO("La orden es girar ptu a la izquierda");
    if(havePTU)
    {
        float currentAngle = ptu->getPan() -15;
        ArLog::log(ArLog::Normal, "Moving PTU to the left 15º...");
        ptu->gotoPos(currentAngle,0);
    }
    else ROS_WARN("No se envia orden %s porque no se ha detectado la PTU",msg->data.c_str());
    esOrdenCorrecta=true;
